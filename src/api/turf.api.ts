...
const coverImageUrl = (cover: RawTurfListItem['cover_image']): string | null => {
...
const toVenueVerification = (raw: RawVenueVerification, venueId = ''): VenueVerification => ({
  id: raw.id ?? '',
...
export const turfApi = {
  listVenues: async (params?: { location?: string; status?: string; search?: string }) => {
    const { data } = await axiosInstance.get<RawVenue[]>(venuesUrl, { params });
    return data.map(toVenueListItem);
  },
  listOwnerVenues: async () => {
    const { data } = await axiosInstance.get<RawVenue[]>(`${venuesUrl}my_venues/`);
    return data.map(toVenueListItem);
  },
  ...
  createVenue: async (data: VenueCreate) => {
    const response = await axiosInstance.post<RawVenue>(venuesUrl, venuePayload(data));
    return toVenue(response.data);
  },
  updateVenue: async (id: string, data: VenueUpdate) => {
    const response = await axiosInstance.patch<RawVenue>(`${venuesUrl}${id}/`, venuePayload(data));
    return toVenue(response.data);
  },
  deleteVenue: (id: string) => axiosInstance.delete(`${venuesUrl}${id}/`),
  getVenueVerification: async (venueId: string) => {
    const { data } = await axiosInstance.get<RawVenueVerification>(`${venuesUrl}${venueId}/verification/`);
    return toVenueVerification(data, venueId);
  },
  submitVenueVerification: async (venueId: string, data: VenueVerificationSubmit) => {
  ...
  listTurfs: async (filters?: TurfFilters) => {
    ...
    const { data } = await axiosInstance.get<RawTurfListItem[]>(turfsUrl, { params });
  ...
  createTurf: async (data: TurfCreate) => {
    const response = await axiosInstance.post<RawTurf>(turfsUrl, turfPayload(data));
    return toTurf(response.data);
  },
  updateTurf: async (id: string, data: TurfUpdate) => {
    const response = await axiosInstance.patch<RawTurf>(`${turfsUrl}${id}/`, turfPayload(data));
    return toTurf(response.data);
  },
  deleteTurf: (id: string) => axiosInstance.delete(`${turfsUrl}${id}/`),
  getTurfAvailability: async (turfId: string, date: string): Promise<TurfAvailability> => {
    const { data } = await axiosInstance.get<TimeSlot[]>(`${turfsUrl}${turfId}/availability/`, { params: { date } });
    return { turf: turfId, date, slots: data.map((slot) => ({ ...slot, bookingId: slot.bookingId ?? null })) };
  },
  listTurfImages: async (turfId: string) => {
    const { data } = await axiosInstance.get<RawTurfImage[]>(`${turfsUrl}${turfId}/images/`);
    return data.map((image) => toTurfImage(image, turfId));
  },
  uploadTurfImages: async (turfId: string, images: File[]) => {
    const { data } = await axiosInstance.post<RawTurfImage[]>(`${turfsUrl}${turfId}/images/`, imageFormData(images));
    return data.map((image) => toTurfImage(image, turfId));
  },
  updateTurfImage: async (turfId: string, imageId: number, data: TurfImageUpdate) => {
    const response = await axiosInstance.patch<RawTurfImage>(`${turfsUrl}${turfId}/images/${imageId}/`, data);
    return toTurfImage(response.data, turfId);
  },
  deleteTurfImage: (turfId: string, imageId: number) => axiosInstance.delete(`${turfsUrl}${turfId}/images/${imageId}/`),
  listTurfReviews: async (turfId: string) => {
    const { data } = await axiosInstance.get<RawTurfReview[]>(`${turfsUrl}${turfId}/reviews/`);
    return data.map((review) => toTurfReview(review, turfId));
  },
  createTurfReview: async (turfId: string, data: TurfReviewCreate) => {
    const response = await axiosInstance.post<RawTurfReview>(`${turfsUrl}${turfId}/reviews/`, data);
    return toTurfReview(response.data, turfId);
  },
  updateTurfReview: async (turfId: string, reviewId: number, data: TurfReviewUpdate) => {
    const response = await axiosInstance.patch<RawTurfReview>(`${turfsUrl}${turfId}/reviews/${reviewId}/`, data);
    return toTurfReview(response.data, turfId);
  },
  deleteTurfReview: (turfId: string, reviewId: number) => axiosInstance.delete(`${turfsUrl}${turfId}/reviews/${reviewId}/`),
};
